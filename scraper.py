...
def get_data(base_url, user_agent = {'User-agent': 'Mozilla/5.0'}):
  ...
  for row_data in games:
      ...
      metascore = row_data.find('a', class_ = 'metascore_anchor').text.strip()
      ...
      df_temp = df_temp.append({'Image' : image['src'], 'Name' :name, 'URL' : 'https://www.metacritic.com'+ url['href'], 'Metascore' : metascore, 'Userscore' : userscore, 'Platform' : platform, 'Summary': summary},  ignore_index = True)
      '''print(f"Name: {name}")
